...
local lastPickup = -2   
local zOffset = -0.65	
local hintToDisplay = "Görüntülenecek ipucu yok"				
...
function refreshBlips()
	if playerData.job.name ~= nil and playerData.job.name == 'Heli' then 	
		drawBlip(Config.locker, 370, "İş Kıyafetlerinizi Giyin")			 	
		drawBlip(Config.carSpawner, 481, "Kargo Helikopterini Çıkar")					
		drawBlip(Config.carDelete, 481, "Kargo Helikopterini Sil")				
	end
end
...
function spawnFork()							
	...
	SetVehicleNumberPlateText(currentCar, "EXCA" .. math.random(1000, 9999))								
...
function goDeliver()
	ESX.ShowNotification('~y~Kargoyu haritada işaretli olan yere teslim edin.')
	setGPS(taskPoints['deliver'])
	currentJob = 'deliver'
end

function goPickup()
	ESX.ShowNotification('~y~Kargoyu konumdan teslim alın.')
...
function getPaid()
	...
	if IsPedInAnyVehicle(GetPlayerPed(-1)) and isMyCar() then			
		...
		ESX.ShowNotification('~w~Kazanç: ~g~ +' .. pay .. ' ~w~TL.')
	else														
		ESX.ShowNotification('~r~Kargo Helikopteri nerede?')
		local pay = math.floor(packetsDelivered * (Config.pay * 0.6))			
		if packetsDelivered < 2 then						
			pay = 0								
		end
		TriggerServerEvent('esx_fork:getPaid', pay)
		ESX.ShowNotification('~w~Kazanç: ~r~ +' .. amount .. ' ~w~TL.')
	end
	currentJob = 'none'											
	currentPlate = ''												
	currentVehicle = nil											
	...
	while true do																
		...
		if not menuIsOpen then
			...
			if playerData.job ~= nil and playerData.job.name == "Heli" and playerIsInside(playerCoords, Config.locker, 2.5) then 				
				isInMarker = true
				displayHint = true																
				hintToDisplay = "Kıyafetlerinizi değiştirmek için E tuşuna basın."									
				currentZone = 'locker'																
			elseif onDuty and taskPoints['deliver'] == nil and playerIsInside(playerCoords, Config.carSpawner, 2.5) then	
				isInMarker = true
				displayHint = true
				hintToDisplay = "İşe başlamak için E tuşuna basın."
				currentZone = 'start'
			elseif onDuty and currentJob == 'deliver' and taskPoints['deliver'] ~= nil and playerIsInside(playerCoords, taskPoints['deliver'], Config.pickupDistance) then
				isInMarker = true
				displayHint = true
				hintToDisplay = "~y~Aldığın kargoyu haritadaki işaretli konuma götür. ~r~Sakın yere düşürme!"
				currentZone = 'none' 
			elseif playerData.job ~= nil and playerData.job.name == "Heli" and currentPlate ~= '' and playerIsInside(playerCoords, Config.carDelete, 1.5) then  				
				isInMarker = true
				displayHint = true
				hintToDisplay = "Kargo helikopterini garaja koymak için E tuşuna basın."
				currentZone = 'pay'
			else																			
				isInMarker = false
				displayHint = false
				hintToDisplay = "Görüntülenecek ipucu yok."
				currentZone = 'none'
			end
...
function openMenu()									
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'locker',			
    {
      ...
      elements = {
        {label = "İş Kıyafetleri", value = 'fork_wear'},		
        {label = "Sivil Kıyafetler", value = 'everyday_wear'}	
      }
    },
    function(data, menu)									
